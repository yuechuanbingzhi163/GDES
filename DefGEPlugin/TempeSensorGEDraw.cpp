...
ACRX_CONS_DEFINE_MEMBERS ( TempeSensorGEDraw, MineGEDraw, 1 )

TempeSensorGEDraw::TempeSensorGEDraw()
{

}

TempeSensorGEDraw::~TempeSensorGEDraw( void )
{
}

void TempeSensorGEDraw::setAllExtraParamsToDefault()
{
	m_radius = 3;
	m_temp = _T("");
}

void TempeSensorGEDraw::configExtraParams()
{

}

void TempeSensorGEDraw::updateExtraParams()
{

}

void TempeSensorGEDraw::writeKeyParam( DrawParamWriter& writer )
{
	writer.writePoint( m_insertPt );
	writer.writeDouble( m_angle );
}

void TempeSensorGEDraw::readKeyParam( DrawParamReader& reader )
{
	reader.readPoint( m_insertPt );
	reader.readDouble( m_angle );
}

void TempeSensorGEDraw::readExtraParam( DrawParamReader& reader )
{
	reader.readDouble( m_radius );
	reader.readString( m_temp );
}

void TempeSensorGEDraw::writeExtraParam( DrawParamWriter& writer )
{
	writer.writeDouble( m_radius );
	writer.writeString( m_temp );
}

Adesk::Boolean TempeSensorGEDraw::subWorldDraw( AcGiWorldDraw* mode )
{
	assertReadEnabled () ;
	...
	DrawMText(mode,pt,m_angle,_T("T"),0.6 * m_radius);
...
Acad::ErrorStatus TempeSensorGEDraw::subTransformBy( const AcGeMatrix3d& xform )
{
	// 插入点变换
...
Acad::ErrorStatus TempeSensorGEDraw::subGetOsnapPoints (
...
Acad::ErrorStatus TempeSensorGEDraw::subGetGripPoints( AcGePoint3dArray& gripPoints, AcDbIntArray& osnapModes, AcDbIntArray& geomIds ) const
{
	assertReadEnabled () ;
...
Acad::ErrorStatus TempeSensorGEDraw::subMoveGripPointsAt( const AcDbIntArray& indices, const AcGeVector3d& offset )
{
	assertWriteEnabled () ;
...
//}
//
//void TempeSensorGEDraw::caclBackGroundMinPolygon( AcGePoint3dArray& pts )
//{
...
void TempeSensorGEDraw::regPropertyDataNames( AcStringArray& names ) const
{
	names.append(_T("温度"));
}

void TempeSensorGEDraw::readPropertyDataFromGE( const AcStringArray& values )
{
	CString strLenth;
