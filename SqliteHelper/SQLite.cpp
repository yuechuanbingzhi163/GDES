...
typedef int (*SQLiteQueryCallback)(void *para, int n_column, char **column_value, char **column_name);
...
BOOL SQLite::ExcuteQuery(LPCTSTR lpSql,QueryCallback pCallBack)
{
	...
	if(sqlite3_exec(m_db, szSql, (SQLiteQueryCallback)pCallBack, NULL, &errmsg) != SQLITE_OK)
	{
		delete[] szSql;
		return FALSE;
	} 
    delete[] szSql;
#else
	if(sqlite3_exec(m_db, lpSql, (SQLiteQueryCallback)pCallBack, NULL, &errmsg) != SQLITE_OK)
	{
		return FALSE;
	} 
#endif
