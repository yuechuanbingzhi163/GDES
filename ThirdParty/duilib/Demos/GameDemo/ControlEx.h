...
public:
	...
	GameListUI(	CPaintManagerUI* pManager) : _root(NULL), m_dwDelayDeltaY(0), m_dwDelayNum(0), m_dwDelayLeft(0), m_pManager(pManager)
    {
...
public:
    ...
    DeskListUI(CPaintManagerUI* pManager) : m_uButtonState(0), m_dwDelayDeltaY(0), m_dwDelayNum(0), m_dwDelayLeft(0), m_pManager(pManager)
    {
        SetItemSize(CSize(182, 152));
        CDialogBuilder builder;
        CContainerUI* pDesk = static_cast<CContainerUI*>(builder.Create(_T("desk.xml"), (UINT)0, 0, m_pManager));
        if( pDesk != NULL ) {
            for(int i = 0; i < 500; ++i) 
            {
                if( pDesk == NULL ) pDesk = static_cast<CContainerUI*>(builder.Create(0, m_pManager));
                if( pDesk != NULL ) {
                    this->Add(pDesk);
