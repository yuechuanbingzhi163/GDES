...
static bool OnLogoButtonEvent(TEventUI* pTEventUI,LPARAM lParam,WPARAM wParam) {
	if( pTEventUI->Type == UIEVENT_BUTTONDOWN ) {
        CControlUI* pButton = pTEventUI->pSender;
        if( pButton != NULL ) {
            CListContainerElementUI* pListElement = (CListContainerElementUI*)(pButton->GetTag());
            if( pListElement != NULL ) pListElement->DoEvent(*pTEventUI);
        }
    }
    return true;
}
...
Node* CFriendsUI::AddNode(const FriendListItemInfo& item, Node* parent)
{
	...
	if (log_button != NULL)
	{
        ...
        log_button->OnEvent += MakeDelegate(&OnLogoButtonEvent,UIEVENT_BUTTONDOWN);
	}
...
{
    if( iIndex == GetCurSel() ) return true;

    // We should first unselect the currently selected item
    if( GetCurSel() >= 0 ) {
        CControlUI* pControl = GetItemAt(GetCurSel());
        if( pControl != NULL) {
