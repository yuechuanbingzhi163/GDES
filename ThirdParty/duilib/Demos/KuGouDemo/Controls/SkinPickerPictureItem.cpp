...
void CSkinPikerPictureItemUI::PaintStatusImage(HDC hDC)
{
	...
	if( (m_uButtonState & UISTATE_PUSHED) != 0 || (m_uButtonState & UISTATE_HOT) != 0) {
		...
		//SIZE bkNameSize = CRenderEngine::GetTextSize(hDC, GetManager(), m_BkName.GetData(), m_iFont, m_uTextStyle);
		//SIZE authorSize = CRenderEngine::GetTextSize(hDC, GetManager(), m_Author.GetData(), m_iFont, m_uTextStyle);
		SIZE bkNameSize = {0,0}, authorSize = {0,0};
		LONG nTextPadding = (m_rcItem.right - m_rcItem.left  - bkNameSize.cx) / 2;
		...
		nTextPadding = (m_rcItem.right - m_rcItem.left - authorSize.cx) / 2;
